...
EVIDENCE = ROOT / ".nxs" / "evidence" / "NXS-P03"
...
def main() -> int:
    ...
    completed = run("python", "-m", "scripts.nxs_guard", "--phase", "NXS-P03", "--json")
    write(
        "preflight",
        {
            "schema_version": "1.0.0",
    ...
    pytest_evidence(
        "identity-primitives",
        ["tests/unit/test_auth_identity.py", "tests/unit/test_auth_passwords.py"],
    )
    pytest_evidence(
        "key-management",
        ["tests/unit/test_auth_keys.py"],
    )
    pytest_evidence(
        "rate-limiting",
        ["tests/unit/test_auth_ratelimit.py"],
    )
    pytest_evidence(
        "config-hardening",
    ...
    pytest_evidence(
        "concurrency",
        ["tests/concurrency/test_auth_concurrency.py"],
    )
    pytest_evidence(
        "context-isolation",
        ["tests/concurrency/test_tenant_concurrency.py"],
    )
    ...
    write(
        "git",
        {
            "schema_version": "1.0.0",
            "phase": "NXS-P03",
            "result": "PASS" if git_branch == "feat/nxs-p03-security-auth" else "FAIL",
            "branch": git_branch,
            "head_sha": git_sha,
