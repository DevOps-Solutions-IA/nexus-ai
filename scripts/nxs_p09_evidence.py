"""Generate NXS-P09 evidence files from real command outputs (never fabricated).
...
EVIDENCE = ROOT / ".nxs" / "evidence" / "NXS-P09"
PHASE = "NXS-P09"
BRANCH = "feat/nxs-p09-messaging"
...
def main() -> int:
    ...
    pytest_evidence(
        "security",
        ["tests/security/test_messaging_security.py"],
    )
    pytest_evidence(
    ...
    pytest_evidence(
        "concurrency",
        ["tests/concurrency/test_messaging_concurrency.py"],
    )
    pytest_evidence(
        "resilience",
        [
    ...
    pytest_evidence(
        "contracts",
        ["tests/contracts/test_messaging_contracts.py"],
    )
