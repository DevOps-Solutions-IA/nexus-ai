...
EVIDENCE = ROOT / ".nxs" / "evidence" / "NXS-P06"
...
def main() -> int:
    print("Generating NXS-P06 evidence:")
    # 1. Preflight (execution guard)
    completed = run("python", "-m", "scripts.nxs_guard", "--phase", "NXS-P06", "--json")
    write(
        "preflight",
        {
            "schema_version": "1.0.0",
    ...
    pytest_evidence(
        "normalization",
        ["tests/unit/test_customer_normalization.py"],
    )
    ...
    pytest_evidence(
        "customer-flows",
        ["tests/integration/test_customer_conversations.py"],
    )
    pytest_evidence(
        "state-validation",
    ...
    pytest_evidence(
        ...
        ["tests/concurrency/test_customer_concurrency.py"],
    )
    pytest_evidence(
        "customer-resilience",
        ["tests/resilience/test_customer_resilience.py"],
    )
    pytest_evidence(
        "customer-security",
        ["tests/security/test_customer_security.py"],
    )
    ...
    write(
        "git",
        {
            "schema_version": "1.0.0",
            "phase": "NXS-P06",
            "result": "PASS" if git_branch == "feat/nxs-p06-customer-conversations" else "FAIL",
            "branch": git_branch,
            "head_sha": git_sha,
