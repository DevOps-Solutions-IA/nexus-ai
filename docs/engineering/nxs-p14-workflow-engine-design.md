...
- **INV-WF-013 — Explicit dependency semantics.** Every immutable version step declares `ALL` or `ANY` dependency mode. `ALL` is the fail-closed default and requires every predecessor to complete; `ANY` is an explicit alternative-branch join that proceeds only after every predecessor resolves and at least one completes. No join mode is inferred from graph shape.
...
| `workflow_version_steps` | UUIDv7 PK; tenant-aware FK to version; same-tenant dependency edges | Type/config/input contract and `dependency_mode` immutable after publication | Unique step key per version; `ALL`/`ANY` check; composite FKs; indexes on version and dependencies; RLS | Cascade only with unpublished draft version; no arbitrary executable payload |
...
Pause stops new claims while allowing an already-authorized attempt to durably report its outcome. Completion while paused does not unlock successors; resume re-evaluates durable dependency readiness. Cancellation is absorbing and fences every later completion or retry commit. Dependency readiness is a pure function of the immutable dependency list and mode, durable predecessor states, and durable run state: `ALL` requires all predecessors `COMPLETED`; `ANY` waits for all predecessors to resolve and then requires at least one `COMPLETED`. Retry decisions use a persisted maximum and an explicit safe error-code allow-list; P14 performs no clock-driven scheduling.

The API is under `/api/v1/workflows` and `/api/v1/workflow-runs`; every input model forbids unknown fields. Step contracts expose `dependency_mode`, defaulting to `ALL`; alternative joins must opt into `ANY` and have at least two dependencies. RBAC uses `workflow:read`, `workflow:execute`, and `workflow:configure`. All lifecycle events use the P04 transactional outbox and strict payload registry. Migration `c5d6e7f8a9b0` creates the schema, tenant-aware composite keys, forced RLS, indexes, and permission catalog delta; corrective migration `d6e7f8a9b0c1` adds the immutable checked dependency mode with a safe `ALL` backfill/default.
