...
- Requirement: `NXS-SCHED-001`, which depends on `NXS-WF-001`. Preconditions: this governance alignment is merged, exact canonical `main` is green, no phase is active, and the generic lifecycle starts P15 on its implementation branch.
- Governance decision: the governance-only branch `chore/nxs-p15-governance-alignment` defines the requirement and implementation contract but must not call `make nxs-start`, create a P15 manifest, acquire the execution lock, or add runtime/migration/API code.
...
- Evidence: `.nxs/evidence/NXS-P15/` only after implementation commands actually execute. This governance PR creates no implementation evidence. Rollback is abandoning/reverting the unmerged governance branch; no migration exists in this task.
- Observability: future implementation emits bounded P04 scheduler lifecycle/occurrence events with safe IDs, timestamps, state, reason and correlation; no credentials, raw authorization, hidden reasoning, or unbounded customer payloads.
- READY: eventual certification is `SCHEDULER CONTRACT + DURABILITY CERTIFIED`, not physical/global exactly-once or disaster-failover certification. It requires all P15 gates, evidence, exact-head GitHub CI/Security, `NXS-SCHED-001` VALIDATED, P15 READY/GO, the next eligible phase computed without activation, and no unauthorized merge/deployment.
