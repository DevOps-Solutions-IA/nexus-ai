...
- Security/concurrency: forced RLS and tenant-aware composite FKs; trusted tenant context; affirmative channel consent; repeated suppression checks; immutable revision/snapshot binding; bounded audience and rate limits; run-first PostgreSQL locks, `SKIP LOCKED`, uniqueness and owner/token compare-and-set fencing. Valkey/process locks may optimize but never authorize.
- Failure criteria: invalid/cross-tenant/oversized audiences, consent and quiet-hour failures, duplicate P15 wake, multi-worker materialization/claims, pause/cancel/unsubscribe races, downstream ambiguity, rate limits, stale owners and outbox failures resolve according to the durable matrices without provider bypass or duplicate logical sends.
