...
- Scope: bounded platform target/peer administration, revision/idempotency/history, internal authenticated resolver, same-transaction placement admission, pinned dialogs, reference config and real SIP proofs in later implementation.
...
- Concurrency: P18 Cell/placement locks precede P11/P19 authority; unique active target, expected revisions, durable route/owner receipts and deterministic suspend/authorize ordering. C01–C25 are future proof obligations, not passing tests.
...
- Evidence: ADR-0100 and `docs/engineering/nxs-p19-sip-edge-scaling-design.md` define AC01–AC30, real SIP/two-edge/two-Cell and PostgreSQL tests, migration/rollback and observability limits. No P19 implementation evidence yet.
