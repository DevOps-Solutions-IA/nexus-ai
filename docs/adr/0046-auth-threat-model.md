...
| Already-issued access token used after logout, revocation, suspension | `PrincipalStateValidator` re-checks live state before ANY tenant scope is granted (session/user/membership/Organization) — 401/403, never 200; recovery restored explicitly | `tests/integration/test_auth_state_validation.py` |
| Unauthorized Organization profile write (org_member PATCH) | `organization:write` enforced on PATCH, `organization:read` on GET, via the canonical AuthorizationService | `tests/integration/test_auth_rbac_api.py` |
